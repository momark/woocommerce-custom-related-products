...
function crp_save_related_products( $post_id, $post ) {
	...
	if ( isset( $_POST['related_ids'] ) ) {
		if ( $woocommerce->version >= '2.3' ) {			
			$fetch = isset( $_POST['related_ids'] ) ? array_filter( array_map( 'intval', explode( ',', $_POST['related_ids'] ) ) ) : array();			
			foreach ( $fetch as $id ) {								
				$translated_product_id = icl_object_id($id, 'product', false,ICL_LANGUAGE_CODE); //Fetch the product ID of current language				
				$related[] = $translated_product_id;
			}
		} else {
			$related = array();
